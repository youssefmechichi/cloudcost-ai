...
export class BillingService {
  ...
  async getMonthlyTrends(userId: string) {
    ...
    for (const record of records) {
      ...
      monthlyTotals[month] = (monthlyTotals[month] || 0) + record.cost;
    }
  ...
  async getForecast(userId: string) {
    ...
    const total = monthlyTrends.reduce((sum, month) => sum + month.cost, 0);

    const averageMonthlySpend = total / monthlyTrends.length;

    const latestMonthSpend = monthlyTrends[monthlyTrends.length - 1].cost;
